...
export function addLiftCollapseButtons(
	...
	const container = d3.select(svgElement)
		.selectAll('g.nodes') as d3.Selection<d3.BaseType, GraphDataNode, Element, unknown>;

	container.filter(d => d.members.length > 0 || (d.originalMembers ?? []).length > 0)
		.append('circle')
	...
	container.filter(d => d.members.length > 0)
		.append('circle')
		.attr('r', drawSettings.buttonRadius)
		.attr('cx', d => -0.5 * d.width! + drawSettings.buttonRadius + drawSettings.nodePadding)
		.attr('cy', d => -0.5 * d.height! + 3 * drawSettings.buttonRadius + 2 * drawSettings.nodePadding)
		.attr('fill', 'blue')
		.attr('fill-opacity', '0.1')
...
export function renderNodeLabels(svgElement: Element, drawSettings: DrawSettingsInterface) {
	if (drawSettings.showNodeLabels) {
		...
		(d3.select(svgElement)
			.selectAll('g.nodes') as d3.Selection<SVGElement, GraphDataNode, Element, undefined>)
			.append('text')
			.text(d => d.id)
 ...
 */
export function renderNodes(nodes: GraphDataNode[], svgElement: Element, drawSettings: DrawSettingsInterface) {
	if (nodes.length === 0) return;

	const level = nodes[0].level;
	const nodeSelection = d3.select(svgElement)
	...
	const rectangle = nodeSelection.select('rect').node() ?
		nodeSelection.select('rect') as d3.Selection<SVGRectElement, GraphDataNode, Element, unknown> :
